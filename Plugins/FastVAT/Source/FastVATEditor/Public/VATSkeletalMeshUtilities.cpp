...
int32 FVATSkeletalMeshUtilities::GetVertices(const UStaticMesh* StaticMesh, const int32 LODIndex,
...
int32 FVATSkeletalMeshUtilities::GetVertices(const USkeletalMesh* SkeletalMesh, const int32 LODIndex, 
...
int32 FVATSkeletalMeshUtilities::GetTriangles(const USkeletalMesh* SkeletalMesh, const int32 LODIndex,
...
int32 FVATSkeletalMeshUtilities::GetNumBones(const USkeletalMesh* SkeletalMesh)
{
	check(SkeletalMesh);
...
*/
void FVATSkeletalMeshUtilities::GetRefBoneTransforms(const USkeletalMesh* SkeletalMesh, TArray<FTransform>& Transforms)
{
	check(SkeletalMesh);
	Transforms.Reset();
...
bool FVATSkeletalMeshUtilities::HasBone(const USkeletalMesh* SkeletalMesh, const FName& Bone)
{
	check(SkeletalMesh);
...
void FVATSkeletalMeshUtilities::GetBoneNames(const USkeletalMesh* SkeletalMesh, TArray<FName>& OutNames)
{
	check(SkeletalMesh);
	OutNames.Reset();
...
FVector3f FVATSkeletalMeshUtilities::PointAtBarycentricCoordinates(const FVector3f& PointA, const FVector3f& PointB, const FVector3f& PointC, const FVector3f& BarycentricCoords)
{
	return (PointA * BarycentricCoords.X) + (PointB * BarycentricCoords.Y) + (PointC * (1.f - BarycentricCoords.X - BarycentricCoords.Y));
};

void FVATSkeletalMeshUtilities::GetSkinWeights(const USkeletalMesh* SkeletalMesh, const int32 LODIndex, 
...
void FVATSkeletalMeshUtilities::InterpolateVertexSkinWeights(const TArray<VertexSkinWeightMax>& SkinWeights, const TArray<float>& Weights,
...
void FVATSkeletalMeshUtilities::ReduceSkinWeights(const TArray<VertexSkinWeightMax>& InSkinWeights, 
...
void FVATSkeletalMeshUtilities::GetSkinnedVertices(const USkeletalMeshComponent* SkeletalMeshComponent, const int32 LODIndex,
	TArray<FVector3f>& OutPositions)
{
...
FVector3f FVATSkeletalMeshUtilities::FindClosestPointToTriangle(const FVector3f& P, const FVector3f& A, const FVector3f& B, const FVector3f& C)
{
...
FVector3f FVATSkeletalMeshUtilities::GetTriangleNormal(const FVector3f& A, const FVector3f& B, const FVector3f& C)
{
...
uint8 FVATSkeletalMeshUtilities::GetTriangleTangentLocalIndex(const FVector3f& P, const FVector3f& A, const FVector3f& B, const FVector3f& C)
{
...
FMatrix44f FVATSkeletalMeshUtilities::GetTriangleMatrix(const FVector3f& P, const FVector3f& A, const FVector3f& B, const FVector3f& C, const uint8 TangentLocalIndex)
{
	// Tangent Vector
	FVector3f V0;
...
FVector3f FVATSkeletalMeshUtilities::BarycentricCoordinates(const FVector3f& P, const FVector3f& A, const FVector3f& B, const FVector3f& C)
{
	const FVector3f V0 = B - A;
	const FVector3f V1 = C - A;
...
void FVATSkeletalMeshUtilities::InverseDistanceWeights(const FVector3f& Point, const TArray<FVector3f>& Points,
...
void FVATSkeletalMeshUtilities::DecomposeTransformation(const FTransform& Transform, 
...
void FVATSkeletalMeshUtilities::DecomposeTransformations(const TArray<FTransform>& Transforms, 
